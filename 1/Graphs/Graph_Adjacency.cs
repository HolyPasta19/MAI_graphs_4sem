...
namespace Graphs
{
    ...
    internal class Graph_Adjacency : Graph_int
    {
        public List<List<int>> graph;
        private int max;
        public Graph_Adjacency(string path)
        {
            int k = 0;
        ...
        public bool Is_Edge(Vertex vi, Vertex vj)
        {
            if (Weight(vi, vj) != 0)
                return true;
            else
        ...
        {
            return graph[v].Select(x => (v,x,1)).ToList();                      
        }
