    ...
    public class Program
    {
        static async Task Main(string[] args)
        {
            ...
            if (graph.IsDirected()==true)
            {
                ...
                if (strongly.Count() == 1)
                    sw.WriteLine("DGraphs_int is strongly connected.");
                else sw.WriteLine($"DGraphs_int is weakly connected and contains {strongly.Count()} strongly connected components.");
