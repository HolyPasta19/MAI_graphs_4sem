...
internal class Program
{
    ...
    static int HeuristicManhattan(Cell a,  Cell b)
    {
        return Math.Abs(b.x-a.x) + Math.Abs(b.y-a.y);
    }
    static int HeuristicChebyshev(Cell a, Cell b)
    {
        return Math.Max(Math.Abs(b.x-a.x), Math.Abs(b.y-a.y));
    }
    static int HeuristicEuclidean(Cell a, Cell b)
    {
        return (int)Math.Sqrt(Pow2(b.x - a.x) + Pow2(b.y - a.y));
    }
    static int HeuristicDijkstra(Cell a, Cell b)
    {
        return 0;
    }
    static void Main(string[] args)
        {
            ...
            flag = 0;
        List<Tuple<string, Func<Cell, Cell, int>>> heuristics = new List<Tuple<string, Func<Cell, Cell, int>>>()
        {
